...
CREATE TABLE roles (
    ...
    role_id INTEGER UNIQUE NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
...
CREATE TABLE objects (
    id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL UNIQUE,
    type VARCHAR(255) NOT NULL DEFAULT 'объект' CHECK (type IN ('объект', 'артефакт', 'технология', 'ресурс', 'декорация', 'оружие', 'магия')),
    description TEXT,
...
INSERT INTO objects (name, type, description, found_location_id) VALUES
    ('Экран монитора', 'технология', 'Устройство для отображения изображений', 1),
...
INSERT INTO events (name, location_id, description) VALUES
    ('Победа Олвина', 1, 'Олвин достигает своей цели'),
    ('Исследование Диаспара', 1, 'Олвин исследует город снаружи');

-- Заполнение таблицы character_event (Эмоции персонажей)
INSERT INTO character_event (character_id, emotion_id, event_id, intensity) VALUES
    (1, 1, 1, 9),  -- Олвин, Радость, событие "Победа Олвина", интенсивность 9
