...
-- Number 6
SELECT DISTINCT "Н_УЧЕНИКИ"."ГРУППА", "Н_ЛЮДИ"."ФАМИЛИЯ", "Н_ЛЮДИ"."ИМЯ", "Н_ЛЮДИ"."ОТЧЕСТВО", "Н_УЧЕНИКИ"."П_ПРКОК_ИД"
FROM "Н_УЧЕНИКИ"
...
WHERE "Н_УЧЕНИКИ"."ЧЛВК_ИД" IS NULL
   OR "Н_УЧЕНИКИ"."ПРИЗНАК" = 'отчисл'
ORDER BY "Н_ЛЮДИ"."ИД";




-- Extra exercise: Для каждой группу посчитать сколько отличников, хорошистов, троечников, двоечников и странных студентов
WITH student_categories AS (
    SELECT
        "Н_УЧЕНИКИ"."ГРУППА",
        "Н_УЧЕНИКИ"."ИД",
        CASE
            ...
            WHEN avg_grade >= 3.5 THEN 'хорошист'
            WHEN avg_grade >= 2.5 THEN 'троечник'
            WHEN avg_grade < 2.5 THEN 'двоечник'
            ELSE 'особый случай'
        END AS "ТИП_СТУДЕНТА"
    FROM "Н_УЧЕНИКИ"
    INNER JOIN (
        SELECT
            ...
            ROUND(AVG(
                CASE
                    WHEN "Н_ВЕДОМОСТИ"."ОЦЕНКА" = '5' THEN 5
                    WHEN "Н_ВЕДОМОСТИ"."ОЦЕНКА" = '4' THEN 4
                    WHEN "Н_ВЕДОМОСТИ"."ОЦЕНКА" = '3' THEN 3
                    WHEN "Н_ВЕДОМОСТИ"."ОЦЕНКА" = '2' THEN 2
                    WHEN "Н_ВЕДОМОСТИ"."ОЦЕНКА" = 'зачет' THEN 5
                    ELSE NULL
                END
            )::numeric, 2) AS avg_grade
        FROM "Н_ВЕДОМОСТИ"
        GROUP BY "Н_ВЕДОМОСТИ"."ЧЛВК_ИД"
    ) AS student_grades ON "Н_УЧЕНИКИ"."ЧЛВК_ИД" = student_grades."ЧЛВК_ИД"
)
SELECT
    student_categories."ГРУППА",
    COUNT(*) FILTER (WHERE student_categories."ТИП_СТУДЕНТА" = 'отличник') AS "Отличников",
    COUNT(*) FILTER (WHERE student_categories."ТИП_СТУДЕНТА" = 'хорошист') AS "Хорошистов",
    COUNT(*) FILTER (WHERE student_categories."ТИП_СТУДЕНТА" = 'троечник') AS "Троечников",
    COUNT(*) FILTER (WHERE student_categories."ТИП_СТУДЕНТА" = 'двоечник') AS "Двоечников",
    COUNT(*) FILTER (WHERE student_categories."ТИП_СТУДЕНТА" = 'особый случай') AS "Особых случаев",
    COUNT(*) AS "Всего"
FROM student_categories
GROUP BY student_categories."ГРУППА"
ORDER BY student_categories."ГРУППА"::int;
